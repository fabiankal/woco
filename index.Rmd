...
layout(legend = list(orientation = "h",sort = FALSE,x = -0.1, y = -0.045),
         images = list(list(  
          source =  base64enc::dataURI(file = "App/logo2.png"), 
          xref = "paper",  
          yref = "paper",
          x=0.95,y=0.955,
...
renderPlotly({
 ...
 if(input$play==TRUE)
  ...
  Nrund<-round(mean(data4$N,na.rm=TRUE),0)
   ...
   p <- ggplot(data4, aes(x=Kategorie, y=Anteil,text=paste("N:",Nrund))) + #
    geom_col(aes(frame=Datum,ids = Kategorie),position = "identity",show.legend = FALSE) + #fill=Kategorie,colour="white",alpha=.75
    #geom_text(aes(label=round(prop*100, digits = 0)), position=position_stack(vjust=0.5), color="black", size=2) +
    #scale_fill_brewer(palette = "RdYlBu", direction=((as.numeric(input$colordirect2)*2)-1)) +  #alternative:Blues
