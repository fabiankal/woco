...
renderPlotly({
  ...
  p <- p %>% layout(title = "",
    ...
    layout(margin=m,annotations = 
            list(x = 1, y = -0.19, text = "Daten: ACPP + WoCo", 
            showarrow = F, xref='paper', yref='paper', 
            xanchor='right', yanchor='auto', xshift=0, yshift=0,
            font=list(size=14, color="#707070"))) %>% 
    layout(annotations = 
            list(x = 1, y = -0.14, text = paste("N ~",Nrund,"pro Welle"), 
            showarrow = F, xref='paper', yref='paper', 
            xanchor='right', yanchor='auto', xshift=0, yshift=0,
            font=list(size=12, color="#707070")))%>% 
    config(locale = 'de',
...
m <- list(l = 40,r = 40,b = 80,t = 40,pad = 4)  
  ...
  g <- ggplotly(p,tooltip = c("fill","y","text")) %>%
layout(showlegend = FALSE,
    ...
    config(locale = 'de',
           displaylogo = FALSE,
           modeBarButtonsToRemove = c("select2d", "lasso2d","autoScale2d","zoom2d","hoverCompareCartesian"),
           toImageButtonOptions = list(format = "png",width = 800,height = 600,scale = 4
        ))
  #g$x$data[[1]]$hoverinfo <- "none"
  #g$x$data[[2]]$hoverinfo <- "none"
