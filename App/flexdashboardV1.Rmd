 ...
 #Observe changes in variables (will run when variables change)
  ...
  for(i in unique(data2$scales))
      ...
      k=k+1
       ...
       labels_st[k]=paste(unique(data_labels$V1[data_labels$scales==i]),sep=", ",collapse=", ")
       labels_en[k]=paste(unique(data_labels$V5[data_labels$scales==i]),sep=", ",collapse=", ")
       labels_ms[k]=paste(unique(data_labels$V6[data_labels$scales==i]),unique(data_labels$V7[data_labels$scales==i]),sep=", ",collapse=", ")
      text[k]=paste('The scale of variable(s)', variables[k], 'goes from',labels_st[k],'to',labels_en[k],'. Missing values:',labels_ms[k], sep=" ", collapse=" ")
