...
epidf =            read.delim("https://raw.githubusercontent.com/fabiankal/woco/master/output/tables/CovidFaelle_Timeline.csv", sep=";", header=TRUE)
...
renderPlotly({
  ...
  labels_ms=0
  text=0
  for(i in 1:length(unique(data2$scales)))
      {
       variables[i]=paste(unique(data2$variable[data2$scales==i]),sep=", ",collapse=", ")
       labels_st[i]=paste(unique(data_labels$V1[data_labels$scales==i]),sep=", ",collapse=", ")
       labels_en[i]=paste(unique(data_labels$V5[data_labels$scales==i]),sep=", ",collapse=", ")
       labels_ms[i]=paste(unique(data_labels$V6[data_labels$scales==i]),unique(data_labels$V7[data_labels$scales==i]),sep=", ",collapse=", ")
      text[i]=paste('The scale of variable(s)', variables[i], 'goes from',labels_st[i],'to',labels_en[i],'. Missing values:',labels_ms[i], sep=" ", collapse=" ")
  }
