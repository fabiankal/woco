...
data_varlab$labels<-gsub(pattern = 'Aktülles', replacement = "Aktuelles",data_varlab$labels)
...
Wählen Sie hier die jeweiligen Variablen aus, die Sie anzeigen möchten.
...
renderPlotly({
  ...
  p <- p %>% layout(title = "",
         ...
         images = list(list(  
          source =  base64enc::dataURI(file = "logo.png"), 
          xref = "paper",  
          yref = "paper",
          x=0.97,y=0.93,
    ...
    config(locale = 'de',displaylogo = FALSE,modeBarButtonsToRemove = c("select2d", "lasso2d","autoScale2d"))
  }
  else
  {
  p <- data2 %>%
...
Wählen Sie hier die Variable aus, die Sie anzeigen möchten.
...
renderPlotly({
  ...
  data3 <- data3 %>% rename(Datum = wavedate,
                            Anteil = prop,
  ...
  ggplotly(p) %>%
layout(legend = list(orientation = "h",sort = FALSE),
         images = list(list(  
          source =  base64enc::dataURI(file = "logo.png"), 
          xref = "paper",  
          yref = "paper",
