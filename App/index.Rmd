...
m <- list(l = 0,r = 0,b = 90,t = 20,pad = 4)  

  g <- ggplotly(p,tooltip = c("x","y","fill","text"),dblclick = FALSE) %>%
layout(legend = list(orientation = "h",sort = FALSE,x = -0.1, y = -0.095),
         images = list(list(  
          source =  base64enc::dataURI(file = "logo2.png"), 
          xref = "paper",  
          yref = "paper",
       ...
       margin=m)  %>% 
    layout(annotations = 
            list(x = 1, y = -0.27, text = "Daten: ACPP + WoCo", 
            showarrow = F, xref='paper', yref='paper', 
            xanchor='right', yanchor='auto', xshift=0, yshift=0,
            font=list(size=14, color="#707070"))) %>% 
    layout(annotations = 
            list(x = 1, y = -0.22, text = paste("N ~",Nrund,"pro Welle"), 
            showarrow = F, xref='paper', yref='paper', 
            xanchor='right', yanchor='auto', xshift=0, yshift=0,
            font=list(size=12, color="#707070")))%>% 
    config(locale = 'de',
