...
#githubdir <-"C:/Users/Kalleitner/Documents/GIT/woco/"
...
renderPlotly({
  ...
  p <- ggplot(data3, aes(fill=Kategorie,x=Datum, y=Anteil)) + 
    ...
    labs(y = "", x= "",fill="", title=paste0("\n",data_labels$labels[data_labels$variable %in% input$var_shares])) +
    theme_minimal() +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    scale_x_date(date_breaks = "3 month",date_labels = "%b %y",guide = guide_axis(n.dodge = 2)) +
...
layout(legend = list(orientation = "h",sort = FALSE,x = -0.1, y = -0.045,tracegroupgap = 0),
         images = list(list(  
          source =  base64enc::dataURI(file = paste0(logodir,"logo2.png")), 
          xref = "paper",  
          yref = "paper",
          x=0.95,y=0.955,
...
renderPlotly({
  ...
  if(input$incidence)
  {
    p <- p  %>% add_trace(x = ~epidf_rc$time, y = ~round((epidf_rc$daily_inf_7T*0.01123461),1), type = 'scatter', mode = 'none', name = '7-Tage Inzidenz',yaxis = "y2", fill = 'tozeroy', inherit = FALSE) %>% layout(yaxis2 = list(overlaying = "y", side = "right",showgrid = F),margin = list(r=38))
...
renderPlotly({
  ...
  plot_ly(x = ~wavedate, y= ~mean_measurement,type = 'scatter',mode = 'lines+markers',linetype=~subgroup, 
          error_y = list(array = ~y_err))#,
          #sort = FALSE)
  p <- p %>% layout(title = list(text=data_labels$labels[data_labels$variable %in% input$variablesgr],x=0.05,y=.97),
         xaxis = list(title = ""),
  ...
  p <- p %>% layout(title = list(text=data_labels$labels[data_labels$variable %in% input$variablesgr],x=0.05,y=.97),
         xaxis = list(title = ""),
         yaxis = list (title = "",rangemode = ifelse(input$axis02,"tozero","normal")),
         title = list(title = paste0("\n",data_labels$labels[data_labels$variable %in% input$variablesgr])),
         legend = list(orientation = 'h'),
         images = list(list(  
          source =  base64enc::dataURI(file = paste0(logodir,"logo2.png")), 
          xref = "paper",  
...
renderPlotly({
  ...
  p <- ggplot(data3, aes(fill=Kategorie,x=Kategorie, y=Anteil,text=paste("N:",Nrund))) + 
    ...
    labs(y = "", x= "",fill="", title=paste0("\n",data_labels$labels[data_labels$variable %in% input$var_shares2])) +
    theme_minimal() +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
...
layout(showlegend = FALSE,
       ...
       margin=m)  %>% 
    layout(annotations = 
            list(x = 0, y = -0.14, text = paste("N ~",Nrund," pro Welle", "Daten: ACPP + WoCo"), 
            showarrow = F, xref='paper', yref='paper', 
