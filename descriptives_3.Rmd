...
# data_plot_long_mean[[2]] <- out[[2]]  %>% filter(!is.na(measurement)) %>%
#   as_survey_design(weights = c(WEIGHTD)) %>%
#   group_by(wavedate,variable) %>%
#   summarise(mean_measurement=survey_mean(abs((as.numeric(measurement)-grouped_labels$min[2])+((grouped_labels$max[2]-grouped_labels$min[2])*(-grouped_labels$rev[2]))),vartype = c("se"))) %>%
#   group_by(wavedate,variable) %>%
